...
export default function CheckinRadar({
  userId,
  venueId,
  onArenaSession,
  autoCheckin,
  ...
  useEffect(() => {
    ...
    apiFetch('/api/auth/is-professional-connector')
      .then((r) => r.json())
      .then((data) => {
        if (!cancelled && data.success) setIsProfessionalConnector(data.isProfessionalConnector);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const handleJoinTable = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        window.alert(
          'Questo tavolo è stato chiuso dal suo Connector — chiedi a chi lo gestisce di riaprirlo.'
        );
      }
    } finally {
        ...
        const newEntries = userIds
          .filter((id) => {
            if (
              id === userId ||
              existingIds.has(id) ||
              seenInThisBatch.has(id) ||
              blockedPairIdsRef.current.has(id)
            )
              return false;
  ...
  async function openScanner() {
    ...
    setTimeout(async () => {
      try {
        ...
        setScannerError(
          'Impossibile accedere alla fotocamera — controlla i permessi, oppure esci e inquadra il QR con la fotocamera del telefono.'
        );
      }
    }, 0);
  }
  ...
  if (status === 'idle' || status === 'checking_in') {
    return (
      <div className="pl-screen" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        {status === 'checking_in' ? (
          <p className="pl-scan-cta" style={{ marginTop: 30, textAlign: 'center' }}>
            Verifica in corso…
          </p>
        ) : (
          <div
          ...
          >
            ...
            {scannerError && (
              <p className="pl-error" style={{ textAlign: 'center' }}>
                {scannerError}
              </p>
            )}
          </div>
  ...
  if (status === 'error') {
    ...
    return (
      <div className="pl-screen">
        ...
        <button className="pl-retry-btn" onClick={() => handleScanQr()}>
          Riprova
        </button>
      </div>
    );
      ...
      {arenaActive ? (
        ...
        <>
          ...
          <div className="pl-counter">
            {checkinCount}/{threshold}
          </div>
          <p>
            I tuoi punti si stanno già accumulando — la classifica si sblocca al raggiungimento
            della soglia.
          </p>
        </>
      )}
      ...
      {arenaSessionId &&
        (tableJoined || isTableConnector ? (
          <div className="pl-hint" style={{ textAlign: 'center', marginTop: 14 }}>
            ...
            {!tableJoined && isTableConnector && (
              <div>
                ✓ Sei il Connector di questo tavolo (gestione PR) — non conti come membro, ma i tuoi
                bonus da Connector restano tuoi.
              </div>
            )}
            ...
            {isTableConnector && (
              <button
                ...
                style={{
                  ...
                  fontSize: 11.5,
                  fontWeight: 700,
                  cursor: tableLockLoading ? 'default' : 'pointer',
                  border: tableLocked
                    ? '1px solid var(--cyan)'
                    : '1px solid rgba(228,212,200,0.25)',
              ...
              >
                {tableLockLoading
                  ? 'Un attimo…'
                  : tableLocked
                    ? '🔒 Tavolo chiuso — tocca per riaprirlo'
                    : '🔓 Chiudi il tavolo a nuovi ingressi'}
              </button>
            )}
          </div>
        ) : !showTableJoin ? (
            ...
            <div className="pl-consent-row">
              <div>
                ...
                <div className="pl-consent-sub">
                  Conta solo se questo tavolo non ha ancora un Connector — altrimenti si eredita chi
                  c'è già
                </div>
              </div>
              <input
                type="checkbox"
                checked={wantsConnector}
                onChange={(e) => setWantsConnector(e.target.checked)}
              />
            </div>
            <button
...
function RadarCard({ personId, arenaSessionId, onClick }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/users/${personId}/public-profile?arenaSessionId=${arenaSessionId || ''}`)
      .then((r) => r.json())
      .then((data) => {
        if (!cancelled && data.success) setPreview(data.profile);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
