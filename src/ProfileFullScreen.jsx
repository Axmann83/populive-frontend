...
export default function ProfileFullScreen({ userId, arenaSessionId, currentUserId, venueId, onClose, viaHistoricalBoard, decisionActions, hideActionButtons }) {
  ...
  const photos = profile
    ? (profile.photoUrls?.length > 0 ? profile.photoUrls : (profile.photoUrl ? [profile.photoUrl] : []))
    : [];
          ...
          <div style={photoContainerStyle}>
            {photos.length > 0 ? (
              ...
              >
                {photos.map((url, i) => (
                  <div
                    key={i}
                    style={{ ...photoImgStyle, scrollSnapAlign: 'start', flexShrink: 0, backgroundImage: `url(${getOptimizedPhotoUrl(url, { width: 600, height: 800, crop: false })})` }}
                    role="img"
            ...
            {photos.length > 1 && (
              <div style={galleryIndicatorWrapStyle}>
                {photos.map((_, i) => (
                  <div key={i} style={{ ...galleryDotStyle, opacity: i === currentPhotoIndex ? 1 : 0.35 }} />
                ))}
              </div>
            )}
