...
export default function VenuesMap({ currentUserId, onClose, mode = 'browse' }) {
  ...
  const createUnofficialCopy = useCallback(
    async (venue) => {
      ...
      if (data.success) {
        window.alert(
          `Fatto — "${venue.name}" è ora attivabile su PopuLive. Puoi già farci check-in.`
        );
        loadVenues();
      } else {
        ...
        if (v.isPartner) {
          ...
          if (v.genderStats) {
            ...
            barRow.style.cssText =
              'display:flex;height:5px;border-radius:999px;overflow:hidden;margin-bottom:8px;';
            barRow.innerHTML = `
          ...
          btn.style.cssText =
            'width:100%;padding:7px 10px;border-radius:8px;border:none;background:#FF3D6E;color:#fff;font-size:10.5px;font-weight:700;cursor:pointer;';
  ...
  useEffect(() => {
    ...
    if (newPinCoords) {
      newPinMarkerRef.current = L.marker(newPinCoords, { icon: NEW_PIN_ICON }).addTo(
        mapRef.current
      );
    }
  }, [newPinCoords]);
    ...
    return (
      <div className="pl-sheet">
        <div
          ...
          onClick={() => setHistoricalVenueId(null)}
          style={{ marginBottom: 4 }}
        >
          ‹ Torna alla mappa
        </div>
        <HistoricalBoard
          venueId={historicalVenueId}
    ...
    if (storiesPeople.length === 0) {
      return (
        <div className="pl-sheet">
          <div
            className="pl-sheet-close"
            onClick={() => setStoriesPeople(null)}
            style={{ marginBottom: 4 }}
          >
            ‹ Torna alla mappa
          </div>
          <p className="pl-hint" style={{ textAlign: 'center', marginTop: 30 }}>
            Nessuno da mostrare per ora — o nessuno è passato di qui negli ultimi giorni, o non ha
            attivato questa opzione.
          </p>
        </div>
      );
    }
      ...
      const res = await apiFetch('/api/venues/create', {
        ...
        body: JSON.stringify({
          ...
          minUsersForLocalRanking:
            Number.isInteger(parsedMinUsers) && parsedMinUsers >= 1 ? parsedMinUsers : undefined,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        <label style={{ fontSize: 9.5, color: 'var(--text-muted)' }}>
          Minimo persone per sbloccare la classifica locale
        </label>
        <input
          type="number"
          min="1"
